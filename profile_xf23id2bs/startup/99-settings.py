...
#   yield from bp.sleep(2)
    ...
    yield from bp.sleep(2)
    yield from bp.abs_set(vortex.mca.rois.roi4.lo_chan, det_settings['vortex_low'], wait=True)
    yield from bp.abs_set(vortex.mca.rois.roi4.hi_chan, det_settings['vortex_high'], wait=True)
...
#        lp_list.append(lp)
    ...
    for j in range(e_scan_params['scan_count']):
        ...
        yield from bp.abs_set(ioxas_x, tmp_pos, wait=True) 
        yield from bp.abs_set(pgm_energy, e_scan_params['start'], wait=True)
        yield from open_all_valves(all_valves)
